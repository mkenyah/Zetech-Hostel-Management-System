...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($service_name)) {
        // Update the service in the database without price
        $stmt = $conn->prepare("UPDATE services SET service_name = ? WHERE id = ?");
        $stmt->bind_param("si", $service_name, $serviceId);

        if ($stmt->execute()) {
            $success = "Service updated successfully!";
            log_action($conn, $_SESSION['user_id'], "Editted a service");
        } else {
...
<div class="d-flex justify-content-center align-items-center min-vh-100">
    <div class="card">
        ...
        <a href="hostel_services.php" class="btn_cancel text-center">Cancel</a>
    </div>
</div>
