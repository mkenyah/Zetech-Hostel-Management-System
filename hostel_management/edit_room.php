<?php
session_start();
include('db.php'); // Ensure this file contains your database connection
include('functions.php');

// Fetch room details if 'id' is passed
if (isset($_GET['id'])) {
    $roomId = $_GET['id'];

    // Fetch room details along with the hostel name
    $stmt = $conn->prepare("SELECT rooms.room_number, rooms.hostel_id, rooms.bed_capacity, hostels.hostel_name 
                            FROM rooms 
                            INNER JOIN hostels ON rooms.hostel_id = hostels.hostel_id 
                            WHERE room_id = ?");
    $stmt->bind_param("i", $roomId);
    $stmt->execute();
    $result = $stmt->get_result();
    $room = $result->fetch_assoc();
    $stmt->close();
}

// Fetch all hostels for the dropdown
$hostelQuery = $conn->query("SELECT hostel_id, hostel_name FROM hostels");
$hostels = [];
while ($row = $hostelQuery->fetch_assoc()) {
    $hostels[] = $row;
}

// Handle form submission
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_room'])) {
    $room_number = isset($_POST['room_number']) ? trim($_POST['room_number']) : '';
    $hostel_id = isset($_POST['hostel_id']) ? trim($_POST['hostel_id']) : '';
    $bed_capacity = isset($_POST['bed_capacity']) ? trim($_POST['bed_capacity']) : '';

    if (!empty($room_number) && !empty($hostel_id) && !empty($bed_capacity)) {
        $stmt = $conn->prepare("UPDATE rooms 
                                SET room_number = ?, hostel_id = ?, bed_capacity = ? 
                                WHERE room_id = ?");
        if ($stmt) {
            $stmt->bind_param("siii", $room_number, $hostel_id, $bed_capacity, $roomId);
            if ($stmt->execute()) {
                // Redirect to rooms list or show success
                header("Location: rooms.php?success=Room updated successfully");
                exit();
            } else {
                $error = "Error updating room. Please try again.";
            }
            $stmt->close();
        } else {
            $error = "Failed to prepare the update statement.";
        }
    } else {
        $error = "All fields are required.";
    }
}
?>




<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Edit Room</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" crossorigin="anonymous">
    <style>
        body {
            background-color: white;
        }
        .card {
            width: 400px;
            background-color: rgb(255, 255, 255);
            border-radius: 10px;
            padding: 20px;
            box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1);
        }
        .form-control {
            background-color: white;
            color: black;
        }
        .btn_update, .btn_cancel {
            background-color: #0f032b;
            color: white;
            width: 100%;
            height: 6vh;
            border-radius: 5px;
            border: none;
            font-weight: bold;
            margin-top: 10px;
        }
        .btn_update:hover, .btn_cancel:hover {
            background-color: rgb(239, 236, 245);
            border: 1px solid #0f032b;
            color: #0f032b;
        }
        a {
            text-align: center;
            text-decoration: none;
            display: block;
            margin-top: 10px;
        }
        label, h4 {
            color: #0f032b;
        }
    </style>
</head>
<body>

<div class="d-flex justify-content-center align-items-center min-vh-100">
    <div class="card">
        <h4 class="fw-bold text-center">Edit Room</h4>
        <?php if (isset($success)) echo "<div class='alert alert-success'>$success</div>"; ?>
        <?php if (isset($error)) echo "<div class='alert alert-danger'>$error</div>"; ?>
        <form method="POST">
            <div class="mb-3">
                <label class="form-label">Room Number</label>
                <input type="text" name="room_number" class="form-control" value="<?php echo htmlspecialchars($room['room_number']); ?>" required >
            </div>
            <div class="mb-3">
                <label class="form-label">Hostel Name</label>
                <select name="hostel_id" class="form-control" required disabled>
                    <option value="">Select a hostel</option>
                    <?php foreach ($hostels as $hostel): ?>
                        <option value="<?= $hostel['hostel_id']; ?>" <?= $room['hostel_id'] == $hostel['hostel_id'] ? 'selected' : ''; ?>>
                            <?= htmlspecialchars($hostel['hostel_name']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                <label class="form-label">Bed Capacity</label>
                <input type="number" name="bed_capacity" class="form-control" value="<?php echo htmlspecialchars($room['bed_capacity']); ?>" required>
            </div>
            <button type="submit" class="btn_update">Update Room</button>
        </form>
        <a href="manage_rooms.php" class="btn_cancel text-center">Cancel</a>
    </div>
</div>

</body>
</html>